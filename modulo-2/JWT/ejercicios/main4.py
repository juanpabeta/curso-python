...
class LoginData(BaseModel):
    ...
    password: str = Field(examples =["MiPasword123"] )
...
def crear_token(data:dict,expiracion: int = ACCESS_TOKEN):
    to_encode = data.copy()
...
@app.get("/")
async def inicio():
    return {"mensaje": "API de autenticación JWT+Passlib"}
...
async def login(datos:LoginData):
    ...
    if not user_pass or not verificar_password(datos.password,user_pass):
        raise HTTPException(status_code= 401,detail = "Crendencial inválida")
